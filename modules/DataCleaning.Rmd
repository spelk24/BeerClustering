...
Are there ingredients that you can combine?
...
ingredients_dummies <- beer_data %>%
  ...
  separate_rows(Ingredients, sep = ",") %>%
  mutate(i1 = 1) %>% 
  pivot_wider(names_from = Ingredients, names_prefix = "ing_", values_from = i1, values_fill = list(i1 = 0)) %>%
  select(-rn)
```
